...
/**
 * Phase 4D-3 — Daily Notification Worker
 *
 * Fired once daily by WorkManager (battery-conscious, no foreground service).
 * Evaluates MORNING_INTENTION, EVENING_REFLECTION, and WEEKLY_REVIEW types.
 *
 * Contract:
...
class DailyNotificationWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            val context = buildNotificationContext(app) ?: run {
                EventLogger.log("NOTIFICATION", "system", "DAILY_WORKER_SKIP",
            ...
            val typesToEvaluate = listOf(
                NotificationType.MORNING_INTENTION,
                NotificationType.EVENING_REFLECTION,
                NotificationType.WEEKLY_REVIEW
            )

            for (type in typesToEvaluate) {
                val decision = SmartNotificationEngine.evaluate(type, context, prefs)
                if (decision is NotificationDecision.Show) {
                    if (governor.canSend(type, prefs)) {
                        ...
                        if (posted) {
                            ...
                            EventLogger.log("NOTIFICATION", "system", "DAILY_NOTIF_SENT",
                                details = "${type.name}: ${decision.candidate.title}")
                        }
                        break // Only one notification per worker run
                    }
                }
            }
            Result.success()
        } catch (e: Exception) {
            EventLogger.log("NOTIFICATION", "system", "DAILY_WORKER_ERROR", details = e.message ?: "Unknown")
            Result.retry()
        }
    }
    ...
    private suspend fun buildNotificationContext(app: DigitalDisciplineApp): NotificationContext? {
        ...
        val isSelfMode = userMode == UserMode.SELF.name
        if (!isSelfMode) return null

        val today = SimpleDateFormat("yyyy-MM-dd", Locale.US).format(Date())
        val cal = Calendar.getInstance()

        // Active Self Mode goals
        ...
        // Recent interventions (last 200 events — covers 24h window for active users)
        ...
        val recentInterventionCount = recentInterventions.size
        val rapidReopenCount = recentInterventions.count { it.reopenWithin5Minutes }
        val peakHour = interventions14d.map { it.hourOfDay }
            .groupBy { it }.maxByOrNull { it.value.size }?.key
        val dataPoints = interventions14d.size
        ...
        val reflectionCompletedToday = lastReflectionDate == today

        // Weekly review due?
        val latestReview = app.personalizationRepository.getLatestWeeklyReview()
        val sevenDaysMs = 7 * 24 * 60 * 60 * 1000L
        val weeklyReviewDue = latestReview == null ||
                (System.currentTimeMillis() - latestReview.generatedAt) >= sevenDaysMs
        ...
        return NotificationContext(
            ...
            distractionDataPoints = dataPoints,
            recentInterventionCount = recentInterventionCount,
            rapidReopenCount = rapidReopenCount,
